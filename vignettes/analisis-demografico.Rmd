...
personas <- get_personas(
  departamento = "Beni",
  variables    = c("p25_sexo", "p26_edad", "g_edad", "nivel_edu",
                   "p40_lee", "p32_pueblo_per", "urbrur")
) |>
  collect() |>
  etiquetar_valores()
...
piramide <- personas |>
  ...
  mutate(
    ...
    n_pir = ifelse(as.integer(p25_sexo) == 1, -1L, 1L)
  ) |>
...
## Alfabetismo por área urbano/rural

```{r alfabetismo}
alfa <- personas |>
  filter(!is.na(p40_lee), !is.na(urbrur), p26_edad >= 15) |>
  count(urbrur, p40_lee) |>
  group_by(urbrur) |>
  mutate(pct = n / sum(n) * 100)

ggplot(alfa, aes(x = urbrur, y = pct, fill = p40_lee)) +
  geom_col() +
  scale_fill_manual(values = c("Sí" = "#003087", "No" = "#d73027"),
                    na.value = "grey80") +
  labs(
    title   = "Alfabetismo (15+ años) por área — Beni, CPV-2024",
    x       = "Área",
    y       = "Porcentaje (%)",
