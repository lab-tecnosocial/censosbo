...
El formato **Arrow** (por defecto) nunca carga todo en RAM: los datos permanecen en el archivo Parquet del disco hasta que ejecutas `collect()`. Todas las tablas se pueden unir por la clave `idep + iprov + imun + i00` (identificador de hogar).
...
```{r ejemplo-piramide}
# Pirámide de edad por grupos quinquenales
...
Los datos se descargan una sola vez y se guardan localmente. Por defecto el caché va al directorio del sistema, pero puedes redirigirlo a tu proyecto:
