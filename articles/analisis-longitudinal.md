# Análisis longitudinal entre censos (1976–2024)
...
#> 3   persona  edad5 GEDAD    P29           P25             <NA>
...
#> 24 vivienda    v09   V09    V15           P09      v09_energia
#> 25 vivienda   v081   V08    V12          <NA>      v15_servsan
#> 26 vivienda    v14   V14    V21           P19     v17_tenencia
#> 27 vivienda    v10   V10    V18           P12      v13_habitac
#>                                                                                                                                                                                                                                                                                                                                                                                                                                                                      notas
...
#> 16                                                                                                                                                                                                                                                                                                    Solo aplica a mujeres de 12 o más años. Hombres y mujeres menores de edad retornan 0 o NA. 2024: p54_hvtot es el total (existe también p54a/p54b por sexo del hijo).
#> 17                                                                                                                                                                                                                                                                                                                                                                                                          Subconjunto de hijos_nacidos_vivos. Solo aplica a mujeres 12+.
...
#> 24                                                                                                                                                                                                                                                                                               1976/1992: 1=Sí, 2=No. 2001 V15: 5=Sí→1, 6=No→2 (codificación distinta). 2012 P09: 1=Sí privado→1, 2=Sí compartido→1, 3=No→2. 2024: 1-4=cualquier fuente→1, 5=No tiene→2.
#> 25                                                                                                                                                                                                                                                             NO disponible en 2012 (variable no exportada desde REDATAM). 1976: 1/2=Tiene→1, 3=No→2. 1992: 1/2=Tiene (con/sin descarga)→1, 3=No→2. 2001: 1=Tiene→1, 2=No→2. 2024: 1/2=Tiene (solo/compartido)→1, 3=No→2.
...
#> 27                                                                                                                                                                                                                                            Variable numérica en 1976/1992/2001/2012. 2024 v13_habitac está codificada como categorías: 1=Uno, 2=Dos, ..., 8=Ocho o más (se usa el código directamente como número ordinales). Se recomienda usar como variable ordinal.
```
...
edu <- get_longitudinal(grupo = "educacion",
                        anios = c(1992, 2001, 2012, 2024))
...
eco <- get_longitudinal(grupo = "economia",
                        anios = c(1976, 1992, 2001, 2012, 2024))
...
cult <- get_longitudinal(grupo = "cultural",
...
mig <- get_longitudinal(grupo = "migracion",
                        anios = c(1992, 2001, 2012, 2024))
...
fert <- get_longitudinal(grupo = "fertilidad",
                         anios = c(1976, 1992, 2001, 2012, 2024))
...
## Vivienda longitudinal: `get_longitudinal_vivienda()`

Para variables de la **tabla vivienda** existe una función equivalente:

``` r

viv <- get_longitudinal_vivienda(
...
viv_basicos <- get_longitudinal_vivienda(
...
viv_mat <- get_longitudinal_vivienda(
...
# Por nombre
edu_lp <- get_longitudinal(grupo = "educacion",
                            anios = c(2001, 2012, 2024),
                            departamento = "La Paz")

# Por código
viv_sc <- get_longitudinal_vivienda(
  variables    = c("fuente_agua", "energia_electrica"),
  anios        = c(2012, 2024),
...
| grupo_edad | persona | Para 2001, 2012 y 2024 se calcula automáticamente desde la edad individual. |
...
| hijos_nacidos_vivos | persona | Solo aplica a mujeres de 12 o más años. Hombres y mujeres menores de edad retornan 0 o NA. 2024: p54_hvtot es el total (existe también p54a/p54b por sexo del hijo). |
| hijos_sobrevivientes | persona | Subconjunto de hijos_nacidos_vivos. Solo aplica a mujeres 12+. |
...
| energia_electrica | vivienda | 1976/1992: 1=Sí, 2=No. 2001 V15: 5=Sí→1, 6=No→2 (codificación distinta). 2012 P09: 1=Sí privado→1, 2=Sí compartido→1, 3=No→2. 2024: 1-4=cualquier fuente→1, 5=No tiene→2. |
| servicio_sanitario | vivienda | NO disponible en 2012 (variable no exportada desde REDATAM). 1976: 1/2=Tiene→1, 3=No→2. 1992: 1/2=Tiene (con/sin descarga)→1, 3=No→2. 2001: 1=Tiene→1, 2=No→2. 2024: 1/2=Tiene (solo/compartido)→1, 3=No→2. |
...
| habitaciones_total | vivienda | Variable numérica en 1976/1992/2001/2012. 2024 v13_habitac está codificada como categorías: 1=Uno, 2=Dos, …, 8=Ocho o más (se usa el código directamente como número ordinales). Se recomienda usar como variable ordinal. |
