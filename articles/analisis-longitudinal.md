# Análisis longitudinal entre censos (1976–2024)

`censosbo` incluye funciones para comparar variables clave entre todos
los censos bolivianos (1976, 1992, 2001, 2012 y 2024). El resultado es
un data.frame en formato largo (“tidy”) con una columna `anio` que
identifica el censo de origen.

## Variables armonizadas disponibles

``` r

variables_armonizadas()
#>        variable                            etiqueta
#> 1          sexo                                Sexo
#> 2          edad                        Edad en años
#> 3    grupo_edad         Grupos de edad quinquenales
#> 4    parentesco Relación con el/la jefe/a del hogar
#> 5  estado_civil             Estado conyugal o civil
#> 6     sabe_leer                Sabe leer y escribir
#> 7     nivel_edu                Nivel de instrucción
#> 8           pea     Población Económicamente Activa
#> 9           pet       Población en Edad de Trabajar
#> 10         area                 Área urbana o rural
#> 11 departamento                        Departamento
#>                                                                                                                                               descripcion
#> 1                                                                               Sexo del individuo. Harmonizado a 1=Mujer, 2=Hombre para todos los censos
#> 2                                                                                                                    Edad del individuo en años cumplidos
#> 3                                                                                              Grupo de edad en intervalos de 5 años (0-4, 5-9, ..., 80+)
#> 4                                                                                        Parentesco o relación del individuo con el jefe o jefa del hogar
#> 5                                                                                                                        Situación conyugal del individuo
#> 6                                                                                   Indica si el individuo sabe leer y escribir. Harmonizado a 1=Sí, 2=No
#> 7  Nivel educativo más alto alcanzado. Para comparación longitudinal se harmoniza a 4 categorías: 0=Sin instrucción, 1=Primaria, 2=Secundaria, 3=Superior
#> 8                                                                                                           Indicador: si el individuo pertenece a la PEA
#> 9                                                                                                     Indicador: si el individuo está en edad de trabajar
#> 10                                                                                                                  Área de residencia: 1=Urbana, 2=Rural
#> 11                                                                                                                         Código de departamento (01-09)
#>     v1976 v1992  v2001         v2012        v2024
#> 1     p03   P03    P28           P24     p25_sexo
#> 2     p04   P04    P29           P25     p26_edad
#> 3   edad5 GEDAD    P29           P25         <NA>
#> 4     p02   P02    P31           P23 p24_parentes
#> 5     p05   P05    P48           P45   p53_ecivil
#> 6     p10   P10    P36           P35      p40_lee
#> 7  nivela   P12 P39NIV P37A_NIVELNUE    nivel_edu
#> 8     pea  NPEA   <NA>           PEA       fft_19
#> 9     pet  NPET   <NA>           PET        ft_19
#> 10   area  <NA>   <NA>          <NA>         <NA>
#> 11    dep  idep   idep          idep         idep
#>                                                                                                                                                                                                                                                                                                                                                                                                                                                                      notas
#> 1                                                                                                                                                                                                                                                                                                                1976/1992/2001: codificación original 1=Hombre, 2=Mujer (invertida). 2012/2024: 1=Mujer, 2=Hombre. get_longitudinal() harmoniza todo a 1=Mujer, 2=Hombre.
#> 2                                                                                                                                                                                                                                                                                                           2001: P29 re-exportado con .dicx corregido (bug fieldsize exportaba 3 bits en vez de 7). 2012: P25 re-exportado desde .dicx (el .dic binario no la exportaba).
#> 3                                                                                                                                                                                                                                                                                                                                                                                              Para 2001, 2012 y 2024 se calcula automáticamente desde la edad individual.
#> 4                                                                                                                                                                                                                                                                                                                                                                                                           Códigos varían entre censos: consultar codebook_ANIO() por año
#> 5                                                                                                                                                                                                                                                                                                                                                                                         Categorías similares entre censos; verificar codebook para equivalencias exactas
#> 6                                                                                                                                                                                                                                                                                                                                                                           1992 (P10): códigos 7=Sí, 8=No (distinto al resto). get_longitudinal() harmoniza a 1=Sí, 2=No.
#> 7  1976: 'nivela' es var. derivada (1=Ninguno..5=Técnico). 1992: P12 solo cubre quienes asistieron; Ninguno se obtiene combinando con P11 en get_longitudinal(). 2001: P39NIV con códigos reales 11=Ninguno,12=Preescolar,13=Básico,14=Intermedio,15=Medio,16=Primaria,17=Secundaria,18=Licenciatura,19=Técnico,20=Normal,21-23=Otros. 2012: P37A_NIVELNUE usa códigos no secuenciales (1,2,3,9,10,11-18,99). La Ley Avelino Siñani (2010) cambió la nomenclatura en 2012.
#> 8                                                                                                                                                                                                                                                                                                                                                         NO disponible directamente en 2001 (requiere cálculo desde variables de actividad). En 2024: fft_19 codifica PEA
#> 9                                                                                                                                                                                                                                                                                                                                                                                                NO disponible directamente en 2001. Edad mínima puede variar entre censos
#> 10                                                                                                                                                                                                                                                          1976: columna 'area' en tabla poblacion (1=Urbana, 2=Rural). 1992/2001/2012: join automático con vivienda; URBRUR/TURUR usan 1=Urbana, 2=Rural. 2024: en tabla vivienda, no disponible via get_longitudinal().
#> 11                                                                                                                                                                                                                                                                                                    En 1976: columna 'dep' (numérica 1-9). En censos REDATAM: 'idep' se calcula desde REDCODEN via join con munic.parquet; get_longitudinal() lo fuerza automáticamente.
```

Las columnas `v1976`–`v2024` muestran el nombre de la variable en cada
censo. `NA` indica que no está disponible en ese año.

## Función `get_longitudinal()`

``` r

datos <- get_longitudinal(
  variables    = c("sexo", "nivel_edu"),
  anios        = c(1992, 2001, 2012, 2024),
  departamento = NULL   # todo el país
)
datos
```

Argumentos principales:

| Parámetro | Descripción |
|----|----|
| `variables` | Vector con nombres armonizados (de [`variables_armonizadas()`](https://lab-tecnosocial.github.io/censosbo/reference/variables_armonizadas.md)) |
| `anios` | Años a incluir: subconjunto de `c(1976, 1992, 2001, 2012, 2024)` |
| `departamento` | Código `"01"`–`"09"` o nombre del departamento. `NULL` = todo el país |
| `verbose` | Mostrar mensajes de progreso (por defecto `TRUE`) |

## Ejemplo: evolución del nivel educativo

``` r

edu <- get_longitudinal(
  variables = "nivel_edu",
  anios     = c(1976, 1992, 2001, 2012, 2024)
)

edu |>
  count(anio, nivel_edu) |>
  group_by(anio) |>
  mutate(pct = n / sum(n) * 100) |>
  ggplot(aes(x = factor(anio), y = pct, fill = factor(nivel_edu))) +
  geom_col() +
  scale_fill_manual(
    values = c("0" = "#d73027", "1" = "#fdae61", "2" = "#74add1", "3" = "#313695"),
    labels = c("0" = "Sin instrucción", "1" = "Primaria",
               "2" = "Secundaria", "3" = "Superior"),
    name   = "Nivel"
  ) +
  labs(
    title   = "Nivel educativo — Bolivia, 1976–2024",
    x       = "Año de censo", y = "Porcentaje (%)",
    caption = "Fuente: INE Bolivia, censos 1976–2024. Elaborado con censosbo."
  ) +
  theme_minimal(base_size = 12)
```

> **Nota sobre armonización:** El nivel educativo se colapsa
> automáticamente a 4 categorías comparables: `0` = Sin instrucción, `1`
> = Primaria, `2` = Secundaria, `3` = Superior. La Ley Avelino Siñani
> (2010) cambió la nomenclatura en 2012; `censosbo` aplica la conversión
> correspondiente.

## Ejemplo: alfabetismo a lo largo del tiempo

``` r

alfa <- get_longitudinal(
  variables = "sabe_leer",
  anios     = c(1976, 1992, 2001, 2012, 2024)
)

alfa |>
  filter(!is.na(sabe_leer)) |>
  count(anio, sabe_leer) |>
  group_by(anio) |>
  mutate(pct = n / sum(n) * 100) |>
  filter(sabe_leer == 1) |>
  ggplot(aes(x = anio, y = pct)) +
  geom_line(linewidth = 1.2, color = "#003087") +
  geom_point(size = 3, color = "#003087") +
  geom_text(aes(label = sprintf("%.1f%%", pct)), vjust = -0.8, size = 3.5) +
  scale_x_continuous(breaks = c(1976, 1992, 2001, 2012, 2024)) +
  ylim(50, 100) +
  labs(
    title   = "Tasa de alfabetismo — Bolivia, 1976–2024",
    x       = "Año de censo", y = "Porcentaje (%)",
    caption = "Fuente: INE Bolivia, censos 1976–2024."
  ) +
  theme_minimal(base_size = 12)
```

## Ejemplo: distribución por sexo y departamento

``` r

sexo_sc <- get_longitudinal(
  variables    = "sexo",
  anios        = c(1992, 2001, 2012, 2024),
  departamento = "Santa Cruz"
)

sexo_sc |>
  count(anio, sexo) |>
  group_by(anio) |>
  mutate(pct = n / sum(n) * 100) |>
  ggplot(aes(x = factor(anio), y = pct, fill = factor(sexo))) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = c("1" = "#F4C430", "2" = "#003087"),
                    labels = c("1" = "Mujer", "2" = "Hombre"),
                    name   = "Sexo") +
  labs(
    title   = "Distribución por sexo — Santa Cruz, 1992–2024",
    x       = "Año de censo", y = "Porcentaje (%)",
    caption = "Fuente: INE Bolivia."
  ) +
  theme_minimal(base_size = 12)
```

## Variables con limitaciones conocidas

``` r

# Ver las notas de cada variable
variables_armonizadas() |>
  select(variable, etiqueta, notas) |>
  filter(!is.na(notas))
#>        variable                            etiqueta
#> 1          sexo                                Sexo
#> 2          edad                        Edad en años
#> 3    grupo_edad         Grupos de edad quinquenales
#> 4    parentesco Relación con el/la jefe/a del hogar
#> 5  estado_civil             Estado conyugal o civil
#> 6     sabe_leer                Sabe leer y escribir
#> 7     nivel_edu                Nivel de instrucción
#> 8           pea     Población Económicamente Activa
#> 9           pet       Población en Edad de Trabajar
#> 10         area                 Área urbana o rural
#> 11 departamento                        Departamento
#>                                                                                                                                                                                                                                                                                                                                                                                                                                                                      notas
#> 1                                                                                                                                                                                                                                                                                                                1976/1992/2001: codificación original 1=Hombre, 2=Mujer (invertida). 2012/2024: 1=Mujer, 2=Hombre. get_longitudinal() harmoniza todo a 1=Mujer, 2=Hombre.
#> 2                                                                                                                                                                                                                                                                                                           2001: P29 re-exportado con .dicx corregido (bug fieldsize exportaba 3 bits en vez de 7). 2012: P25 re-exportado desde .dicx (el .dic binario no la exportaba).
#> 3                                                                                                                                                                                                                                                                                                                                                                                              Para 2001, 2012 y 2024 se calcula automáticamente desde la edad individual.
#> 4                                                                                                                                                                                                                                                                                                                                                                                                           Códigos varían entre censos: consultar codebook_ANIO() por año
#> 5                                                                                                                                                                                                                                                                                                                                                                                         Categorías similares entre censos; verificar codebook para equivalencias exactas
#> 6                                                                                                                                                                                                                                                                                                                                                                           1992 (P10): códigos 7=Sí, 8=No (distinto al resto). get_longitudinal() harmoniza a 1=Sí, 2=No.
#> 7  1976: 'nivela' es var. derivada (1=Ninguno..5=Técnico). 1992: P12 solo cubre quienes asistieron; Ninguno se obtiene combinando con P11 en get_longitudinal(). 2001: P39NIV con códigos reales 11=Ninguno,12=Preescolar,13=Básico,14=Intermedio,15=Medio,16=Primaria,17=Secundaria,18=Licenciatura,19=Técnico,20=Normal,21-23=Otros. 2012: P37A_NIVELNUE usa códigos no secuenciales (1,2,3,9,10,11-18,99). La Ley Avelino Siñani (2010) cambió la nomenclatura en 2012.
#> 8                                                                                                                                                                                                                                                                                                                                                         NO disponible directamente en 2001 (requiere cálculo desde variables de actividad). En 2024: fft_19 codifica PEA
#> 9                                                                                                                                                                                                                                                                                                                                                                                                NO disponible directamente en 2001. Edad mínima puede variar entre censos
#> 10                                                                                                                                                                                                                                                          1976: columna 'area' en tabla poblacion (1=Urbana, 2=Rural). 1992/2001/2012: join automático con vivienda; URBRUR/TURUR usan 1=Urbana, 2=Rural. 2024: en tabla vivienda, no disponible via get_longitudinal().
#> 11                                                                                                                                                                                                                                                                                                    En 1976: columna 'dep' (numérica 1-9). En censos REDATAM: 'idep' se calcula desde REDCODEN via join con munic.parquet; get_longitudinal() lo fuerza automáticamente.
```

Las variables con `NA` en algún censo se incluyen igualmente con valor
`NA` y un aviso de `cli_warn()`:

- **`edad`**: no disponible directamente en 2012 (la tabla procesada no
  la incluye)
- **`area`** (urbano/rural): en la tabla de vivienda, no de persona —
  `NA` en 2001 y 2024
- **`pea`**, **`pet`**: no disponibles directamente en 1992 y 2001

## Combinar con análisis propio

El resultado de
[`get_longitudinal()`](https://lab-tecnosocial.github.io/censosbo/reference/get_longitudinal.md)
es un tibble normal y puede procesarse con cualquier función de R:

``` r

datos <- get_longitudinal(
  variables = c("sexo", "nivel_edu", "sabe_leer"),
  anios     = c(1992, 2001, 2012, 2024)
)

# Tabla resumen
datos |>
  group_by(anio) |>
  summarise(
    n              = n(),
    pct_alfabeto   = mean(sabe_leer == 1, na.rm = TRUE) * 100,
    pct_superior   = mean(nivel_edu == 3, na.rm = TRUE) * 100
  )

# Exportar a CSV
datos |>
  count(anio, nivel_edu) |>
  readr::write_csv("educacion_longitudinal.csv")
```

## Trabajar directamente con los datos por censo

Si necesitas variables no armonizadas o mayor control, accede
directamente a cada censo y combina manualmente:

``` r

# Recuento de nivel educativo por año (columnas distintas por censo)
edu_2012 <- get_personas_2012() |>
  count(nivel = P37A_NIVELNUE) |>
  collect() |>
  mutate(anio = 2012L)

edu_2024 <- get_personas_2024() |>
  count(nivel = nivel_edu) |>
  collect() |>
  mutate(anio = 2024L)

bind_rows(edu_2012, edu_2024)
```

Para ver los nombres de columnas disponibles en cada censo, usa el
codebook:

``` r

# Variables de persona en el censo 2012
codebook_2012(tabla = "persona")

# Buscar variable de nivel educativo en 1976
codebook_1976(buscar = "instruccion")
```
