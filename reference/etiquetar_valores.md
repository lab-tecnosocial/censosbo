...
Con datos de \[get_temporal()\] / \[get_temporal_vivienda()\] se usan
las etiquetas \*\*armonizadas\*\* (no las de un censo concreto). Esto se
detecta por la columna \`anio\`; conviene conservarla al resumir. Para
variables cuyo nombre también existe como columna cruda (\`nivel_edu\`,
\`area\`, \`pea\`, \`pet\`), la columna \`anio\` es necesaria para
